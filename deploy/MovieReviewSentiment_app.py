...
def mrs_strength_to_color_string(
    strength: float, min_str: float, max_str: float
) -> str:
    ...
    strength = max(
        min_str, min(max_str, strength)
    )  # clamp strength to min < strength < max
    color_str = "#"
    if strength < 0:
        color_str += "FF"  # red
...
def mrs_reasoning_html_from_string(data: str) -> str:
    ...
    for n, (prev_word, this_word, next_word) in enumerate(
        zip(data_list[:-2], data_list[1:-1], data_list[2:])
    ):
...
def mrs_demo_prediction():
    ...
    if request.method == "POST":
        ...
        transformed_data = mrs_vocab.transform(
            [data]
        )  # Transform the input string with the HashedVectorizer
        sentiment = mrs_model.predict_proba(transformed_data)[
            0, 1
